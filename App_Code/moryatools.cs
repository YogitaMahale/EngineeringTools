...
public class moryatools : System.Web.Services.WebService
{
        ...
        try
        {
            ...
            if (dtProduct != null)
            {
                if (dtProduct.Rows.Count > 0)
                {
                    ...
                    {
                        dt3 = new DataTable();
                        SqlCommand cmd = new SqlCommand("SELECT 'http://et.engineeringtools.co.in/uploads/product/front/' +  imagevideopath as imagevideopath FROM productimagesvideos Where isdelete=0 and pid=" + ProductId, con);
    ...
    {
        common ocommon = new common();
        string OrderLink = "http://et.engineeringtools.co.in/orderinvoice.aspx?oid=" + ocommon.Encrypt(OrderId.ToString(), true);
        string oSB = string.Empty;
        SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["cnstring"].ConnectionString);
        DataSet ds = new DataSet();
        SqlDataAdapter da;
    ...
    public void Select_OrderTrackImageUsingOrderId(Int64 OrderId)
    {
        ...
        SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["cnstring"].ConnectionString);
        SqlCommand cmd = new SqlCommand();
        cmd.CommandText = "Select top 2 'http://et.engineeringtools.co.in/uploads/orderimage/'+adminimage from orderimagemaster where adminimage is not null and oid=" + OrderId + " order by createddatetime desc ";
        cmd.CommandType = CommandType.Text;
        cmd.Connection = con;
        con.Open();
